...
def duplicates(items):
    ...
    for item in items:
        if item in dupe_counter:
            dupe_counter[item] += 1
        else:
            dupe_counter[item] = 1

    for item in dupe_counter:
        if dupe_counter[item] > 1:
            all_dupes.append(item)

    return sorted(all_dupes)
    ...
    for word in words:
        ...
        if letter in word:
            for character in word:
                if character == letter:
                    letter_indices.append(index_count)
                    break
                index_count += 1
